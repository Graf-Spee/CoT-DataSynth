...
截至 **2026-08-02**，这套 `Self-Instruct baseline` 已经可以作为一个可投入使用的 baseline 来跑实验：

- step 1 -> step 4 已经接入 `DataObs`
- 不是 stub 假通，而是真实调用本地 `Qwen + vLLM`
- 可以产出：
...
如果后面还要继续做工作，主要已经不是“能不能用”的问题，而是：
...
如果你现在要把它当 baseline 去跑实验，可以直接用。

当前更准确的说法是：

- 这已经是一个可用的 `Self-Instruct baseline`
- 不是“还没做完所以不能跑”
- 后面继续做的，主要是：
  - 更细的官方行为对齐
  - 不同数据集上的规模参数校准
  - 和其他 baseline 的最终数据量 / 训练口径对齐
...
截至 **2026-08-02**，这套 baseline 的状态可以概括为：

- DataObs 工程接线已经打通
- 可以在 experiment pipeline 里通过 `--distill-method self_instruct` 调起
- step 2 和 step 3 都已经接到本地 teacher / vLLM
- step 4 已经能把生成结果整理成 DataObs 可训练格式
- step 1 已经改成“官方 bootstrap 结构 + 本地 teacher”的形式
- 目前已经可以把它当成一个可投入使用的 baseline，而不是开发中的 stub
- `gpu6` 上的小规模真实 smoke 已经跑通 step 1 -> step 4
- 最新 smoke 中已经出现：
  - `generation_method = self_instruct_bootstrap`
  - `classification_detection_method = official_template_vllm`
  - `instance_generation_method = self_instruct_instance_teacher`
- 在 `2026-07-31` 的 `step 3 + step 4` 真实 GPU smoke 中：
  - 3 条 `generated_items` 全部成功生成
  - 在 step 4 的最小解析前适配后，最终拿到了 3 条 `sft_rows`
- 在后续收紧 classification allowed-label 约束后：
  - `Positive or Negative` 这类 instruction 不再越界产出 `Mixed`
  - 最新真实 smoke 里已经稳定产出合法标签 `Negative`
- 说明这条链路已经不再只是 stub/fallback 串起来的假通路
- 其中 step 1 / step 2 / step 3 现在都可以按“官方等价迁移”理解
- step 4 的代码这次已经继续往官方 `prepare_for_finetuning.py` 收紧了一步
- 旧的 `step 3` 真实 smoke 产物确实暴露过一个问题：
  - `raw_instances` 更像“直接解题答案”
  - 不是官方 step 4 最偏好的 `Example / Input / Output` 格式
- 但在 `2026-07-31` 的最新收紧后：
  - 已经通过 step 4 解析前的最小 `Output:` 适配把这类 non-classification 结果接回新版 step 4
  - 当前真实 smoke 不再掉成 0 行
- 所以当前剩下的主要工作，已经从“step 4 吃不下 step 3 输出”
- 收敛成“继续观察还要不要把 step 3 的原始生成本身再往官方格式收得更紧”

你可以把现在的实现理解成：

- 框架层已经通了
- 四步结构已经对齐出来了
- step 1 / 2 / 3 都已经能真实调用本地 teacher
- prompt 模板大体已经迁过来了
- step 4 的解析、过滤和编码已经更接近官方了
- 剩下主要是继续验证并收紧 step 3 -> step 4 之间的真实格式衔接
...
- 允许做最小改动
- 总目标是尽可能复现官方 Self-Instruct 的效果
- 不要求机械地保留 GPT / OpenAI completion 的原始外壳
- 允许把原本偏 GPT 风格的 prompt，改写成更适合 Qwen / chat model 的模板形式

这里的关键不是“随意改 prompt”，而是：
...
- 官方 prompt 内容尽量保留
- prompt 包装方式改成 Qwen 更稳定的 chat 模板
- 输出格式约束改成更利于本地模型稳定生成和解析的形式

## 四步差异审计

这里专门按“必须保留的适配 / 应该继续收掉的本地实现”做一次总审计。
...
- 本地模型接入
- Qwen chat wrapper
- classification 的 allowed-label 本地约束

当前基本已经收干净：

- 主体 prompt 逻辑、stop、batch 内 token 规则都已贴官方
- 原始模型输出已经继续原样保留
...
- DataObs parquet 输出格式
- generation task 在解析前的最小 `Output:` 适配

当前基本已经收干净：

- 解析、去重、截断、采样、编码主体已经按官方思路走
- 当前最大差异主要是最终导出目标不是官方 jsonl，而是 DataObs parquet

## 现在最值得注意的限制

1. 这还不是和官方 repo 完全逐字等价的实现。
2. 最大差异点目前仍然是后端：
   - 官方用的是 OpenAI completion 接口
   - 这里用的是本地 chat 模型 + vLLM
3. step 1 的 bootstrap 对模型输出格式很敏感，chat 模型容易先说开场白，再开始列任务。
4. 目前每个阶段都会各自初始化一次 `LocalTeacher`，所以小样本 smoke 也会比较慢。
5. 如果环境里没有 `rouge_score`，step 1 现在不会再走 fallback scorer，而是无法完成真实 bootstrap。
6. step 3 当前虽然已经是真实 teacher generation，但默认只请求 1 个 completion，距离官方更完整的候选保留逻辑还有差距。
7. step 3 在当前 seed task / Qwen 行为下，generation task 仍然可能直接输出“题目答案”，而不是天然自带 `Output:` 前缀。
8. 当前已经在 step 4 解析前用最小 `Output:` 适配接住这类输出，但这依然属于本地 chat-model 适配，不是官方 completion 原生行为。
9. classification task 现在虽然通过 allowed-label 约束更稳了，但这层约束本质上也还是本地 chat-model 适配。
10. step 4 当前已经更像官方 `prepare_for_finetuning`，但最终输出仍然必须保留 DataObs 需要的 parquet 字段，所以不会和官方 jsonl 终稿完全一样。

## 还没完全迁到官方逻辑的点

如果要非常严格地说“哪些地方还不是官方逻辑迁移”，目前主要剩这些：

1. step 1：
   - prompt 外面加了 Qwen chat wrapper
   - 本地 vLLM 返回结果会先包成官方近似 `response` 结构
   - 为了兼容本地 reasoning/chat 模型，还额外去掉了 `<think>...</think>` 块
   - 保留了调试开关 `--bootstrap-allow-stub-fallback`，但默认路径已经不依赖它
2. step 2：
   - 后端不是官方 OpenAI completion
   - `Yes/No` 解析器和停止规则是本地适配版
3. step 3：
   - 主体代码已经基本贴到官方 `generate_instances.py`
   - 但本地 chat model 的真实输出格式仍带一点最小适配层
4. step 4：
   - 现在已经在做更完整的官方式解析、过滤、去重和编码
   - 但最终导出目标是 DataObs parquet，不是官方原始 finetune jsonl

## 这次验证到的一个很重要的现象

`2026-07-30` 在 `gpu6` 上跑的 smoke 里，第一条真实生成样本是这样的：

- step 1 bootstrap 出了一个新的数学 instruction
- step 2 把它判断成 `No`，即非 classification
- step 3 真实生成了一条完整解答
- step 4 把它编码成了可训练的 `prompt/completion`

这说明现在的实现已经从：

- “结构上像四步，但主要靠 fallback 才能跑”

推进到了：

- “四步都能真实调本地 teacher，只是还要继续把行为往官方 repo 细节对齐”

但 `2026-07-31` 又多做了一轮真实 GPU 验证后，情况进一步变成：

- classification 样本已经能被新版 step 4 正常解析
- non-classification 样本即使是自由 prose，只要在 step 4 解析前补上最小 `Output:` 前缀
- 也已经能被新版 step 4 正常解析并落成训练行

所以这个现象现在更准确的理解应该是：

- step 4 代码已经更贴官方了
- step 3 的原始 chat 输出仍然不是官方 completion 风格
- 但目前已经有一层足够克制的本地适配，能把真实 smoke 接通
...
2. `common.py`
3. `templates.py`
4. `bootstrap_instructions.py`
5. `identify_clf_or_not.py`
6. `generate_instances.py`
7. `prepare_for_finetuning.py`

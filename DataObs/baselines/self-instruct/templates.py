...
BOOTSTRAP_PROMPT = "Come up with a series of tasks:"
BOOTSTRAP_PROMPT_CLF = "Come up with a series of classification tasks. Try to specify the possible output labels when possible."
BOOTSTRAP_QWEN_CHAT_TEMPLATE = """You are generating new task instructions by continuing an existing numbered list.

Requirements:
- Continue the list starting from {next_index}.
- Output only the continuation lines of the numbered list.
- Start the first new line exactly with "{next_index}."
- Each line should contain exactly one new task instruction.
- Do not repeat any existing task.
- Do not include any preface, explanation, markdown, or answers.

Numbered list:
{prompt}
"""

CLASSIFICATION_TEMPLATE_1 = """Can the following task be regarded as a classification task with finite output labels?
...
CLASSIFICATION_QWEN_CHAT_TEMPLATE = """You are deciding whether a task is a classification task with a finite set of output labels.

Follow the examples below and then answer the final query.
...
INSTANCE_GEN_QWEN_CHAT_TEMPLATE = """You are generating training examples for the final task below.

Rules:
- Continue the prompt after the final `Task:` line
- Follow the style of the examples already shown in the prompt
- Output only the continuation for the final task
- Do not add any explanation or preface
- If the task does not need extra input, start directly with `Output:`
- Otherwise start with `Example 1` and use `Input:` / `Output:` examples
- Do not directly solve the task in free-form prose

{template} {instruction}
"""

INSTANCE_CLF_QWEN_CHAT_TEMPLATE = """You are generating examples for the final classification task below.
...
- Output only the continuation for the final task
- Do not add any explanation or preface
- Start directly with `Class label:`
- Prefer the exact `Class label:` style shown in the prompt
- If the final task explicitly names a label set, you must use only those labels
- Do not reuse labels from earlier examples unless they are also valid for the final task
